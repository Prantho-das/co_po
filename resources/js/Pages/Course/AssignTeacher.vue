<template>
    ...
    <BreezeAuthenticatedLayout>
        <template #header> Assign Course To Teacher </template>
       ...
       <div class="overflow-hidden my-8 w-full rounded-lg border shadow-xs mb-4">
             <Link :href="route('course.assignTeacherCreate')" class="inline-block m-2 text-md font-weight-bold p-2 rounded bg-green-500 text-white">
           Assign Teacher
    </Link>
            <h2 class="capitalize text-xl font-weight-bold m-2">Running Courses</h2>
            <div class="overflow-x-auto w-full">
                ...
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tbody class="bg-white divide-y">
                        <tr v-for="(run, i) in running.data" :key="run.id" class="text-gray-700">
                            <td class="px-4 py-3 text-sm">
                                {{ i + 1 }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                               {{  run.rel_teacher.name  }}
                            </td>
                            <td class="px-4 py-3 text-sm">
       ...
       <div class="overflow-hidden my-8 w-full rounded-lg border shadow-xs">
            <h2 class="capitalize text-xl font-weight-bold m-2">Completed Courses</h2>
            <div class="overflow-x-auto w-full">
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                    ...
                    <tbody class="bg-white divide-y">
                        <tr v-for="(comp, i) in completed.data" :key="comp.id" class="text-gray-700">
                            <td class="px-4 py-3 text-sm">
                                {{ i + 1 }}
                            </td>
                            <td class="px-4 py-3 text-sm">
                               {{  comp.rel_teacher.name  }}
                            </td>
                            <td class="px-4 py-3 text-sm">
