            ...
            <form @submit.prevent="examCreate" class="mb-4">
                ...
                <div class="my-4">
                    ...
                    <BreezeInput id="name" type="text" class="block w-full mt-1 p-1.5 border-2 border-rose-100"
                        v-model="form.name" required autocomplete="off" />
                    <h2 class="text-red-500" v-if="form.errors.name">{{ form.errors.name }}</h2>
                </div>
                <div class="my-4">
                    <BreezeLabel for="mark" value="Marks" />
                    <BreezeInput id="mark" type="number" class="block w-full mt-1 p-1.5 border-2 border-rose-100"
                        v-model="form.marks" required autocomplete="off" />
                    <h2 class="text-red-500" v-if="form.errors.marks">{{ form.errors.marks }}</h2>
                </div>
                 <div class="my-4">
                    <BreezeLabel for="copo" value="Co Po" />
                    <select id="copo" v-model="form.copo"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                        <option selected value="" disabled>Choose a Co Po</option>
                        <option class="text-black" v-for="copo, i in copos" :key="i" :value="copo.id">{{ copo.rel_co.co_name +' | '+ copo.rel_po.po_name }}</option>
                        <h2 class="text-red-500" v-if="form.errors.copo">{{ form.errors.copo }}</h2>
                    </select>
                </div>
                ...
                <div class="overflow-x-auto w-full">
                    <table class="w-full whitespace-no-wrap">
                        ...
                        <tbody class="bg-white divide-y">
                            <tr v-for="(course, i) in courses.data" :key="course.id" class="text-gray-700">
...
export default {
    ...
    methods: {
        examCreate() {
            this.form.post(this.route("course.teacher.makeExamStore",this.tcourse.id), { onSuccess: () => this.form.reset(), });
        },
    },
    props: {
        copos: Array,
